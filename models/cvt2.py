...
class Attention(nn.Module):
    ...
    def forward_conv(self, x, t, h, w):
        if self.with_cls_token:
            cls_token, x = torch.split(x, [1, t*h*w], 1)
        ...
        if self.conv_proj_q is not None:
            ...
            q = rearrange(x, 'b c t h w -> b (t h w) c')

        if self.conv_proj_k is not None:
            k = self.conv_proj_k(x)
        else:
            k = rearrange(x, 'b c t h w -> b (t h w) c')

        if self.conv_proj_v is not None:
            v = self.conv_proj_v(x)
        else:
            v = rearrange(x, 'b c t h w -> b (t h w) c')
    ...
    def forward(self, x, t, h, w):
        if (
            ...
            q, k, v = self.forward_conv(x, t, h, w)

        q = rearrange(self.proj_q(q), 'b t (h d) -> b h t d', h=self.num_heads)
        k = rearrange(self.proj_k(k), 'b t (h d) -> b h t d', h=self.num_heads)
        v = rearrange(self.proj_v(v), 'b t (h d) -> b h t d', h=self.num_heads)

        attn_score = torch.einsum('bhlt,bhtk->bhltk', [q, k]) * self.scale
        attn_score = attn_score.sum(dim=-1)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.cls_token is not None:
            cls_tokens, x = torch.split(x, [1, T*H*W], 1)
        x = rearrange(x, 'b (t h w) c -> b c t h w', t=T, h=H, w=W)
...
# Now you can define the spec configuration with the desired parameters
